...
class Player_Quests
  ...
  def activateQuest(quest,color,story)
    if !quest.is_a?(Symbol)
      raise _INTL("Das Argument 'Quest' sollte ein Symbol sein, z. B. ':Quest1'.")
    end
    for i in 0...@active_quests.length
      if @active_quests[i].id == quest
        pbMessage("Du hast diese Quest bereits angefangen.")
        return
      end
    end
    for i in 0...@completed_quests.length
      if @completed_quests[i].id == quest
        pbMessage("Du hast diese Quest bereits beendet.")
        return
      end
    end
    for i in 0...@failed_quests.length
      if @failed_quests[i].id == quest
        pbMessage("Du hast diese Quest bereits verbockt.")
        return
      end
    end
    @active_quests.push(Quest.new(quest,color,story))
    pbMessage(_INTL("\\se[{1}]<ac><c2=#{colorQuest("red")}>Neue Quest erhalten!</c2>\nSchaue in dein Questbuch für mehr Informationen!</ac>",QUEST_JINGLE))
  end
  ...
  def failQuest(quest,color,story)
    ...
    for i in 0...@active_quests.length
      if @active_quests[i].id == quest
        ...
        found = true
        pbMessage(_INTL("\\se[{1}]<ac><c2=#{colorQuest("red")}>Quest fehlgeschlagen!</c2>\nDein Questbuch wurde geupdated!</ac>",QUEST_FAIL))
        break
      end
    end
    if !found
  ...
  def completeQuest(quest,color,story)
    ...
    for i in 0...@active_quests.length
      if @active_quests[i].id == quest
        ...
        @active_quests.delete_at(i)
        found = true
        pbMessage(_INTL("\\se[{1}]<ac><c2=#{colorQuest("red")}>Quest abgeschlossen!</c2>\nDein Questuch wurde geupdated!</ac>",QUEST_JINGLE))
        break
      end
    end
    if !found
  ...
  def advanceQuestToStage(quest,stageNum,color,story)
    ...
    for i in 0...@active_quests.length
      if @active_quests[i].id == quest
        ...
        found = true
        pbMessage(_INTL("\\se[{1}]<ac><c2=#{colorQuest("red")}>Neue Aufgabe hinzugefügt!</c2>\nDein Questbuch wurde geupdated!</ac>",QUEST_JINGLE))
      end
      return if found
    end
    if !found

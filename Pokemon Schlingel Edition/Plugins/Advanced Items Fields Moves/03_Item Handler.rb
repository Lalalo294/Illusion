...
def pbCheckForBadge(badge)
  ...
  #pbMessage(_INTL("You dont have the enough bagdes, to use this right now."))
  return false
end
...
def pbCheckForSwitch(switches)
  ...
  switches.each { |switch|
    if !$game_switches[switch]
      #pbMessage(_INTL("You don't meet the requirement to use this item yet."))
      return false
    end
  }
  return true
...
if Item_RockClimb[:active]
  ...
  def aifmRockClimb
    if !pbCanUseItem(Item_RockClimb)
      pbMessage(_INTL("A wall of mighty rock is in front of you."))
      return false
    end
    if pbConfirmMessage(_INTL("A wall of mighty rock is in front of you.\nWould you like to use Rock Climb?"))
  ...
  ItemHandlers::UseFromBag.add(Item_RockClimb[:internal_name], proc do |item|
    if !$game_player.pbFacingTerrainTag.can_climb
      pbMessage(_INTL("You are not facing a rock wall.[BAG]"))
      next 0
    end
    next 2
  end)
  ...
  ItemHandlers::UseInField.add(Item_RockClimb[:internal_name], proc do |item|
    if !$game_player.pbFacingTerrainTag.can_climb
      pbMessage(_INTL("You are not facing a rock wall.[FIELD]"))
      next 0
    end
    aifmRockClimb
    next 1
